...
func (flashes FlashGrid) flashReset() {
	for row := range flashes {
		for col := range flashes {
			flashes[row][col] = false
		}
	}
}

// loop through all the flashes and check if the entire grid flashed
func (flashes FlashGrid) fullFlash() bool {
	for row := range flashes {
		for col := range flashes {
			if !flashes[row][col] {
				return false
	...
	// if we get here, the entire grid flashed
	return true
}
